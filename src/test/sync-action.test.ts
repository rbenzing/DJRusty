...
describe('syncToDeck — exact tempo + phase align', () => {
  beforeEach(() => {
    ...
    playerRegistry.unregister('A'); playerRegistry.unregister('B');
  });

  it('sets this deck pitch to the EXACT continuous ratio (not snapped) and marks synced', () => {
    playerRegistry.register('A', mockPlayer(1.0) as never);
    playerRegistry.register('B', mockPlayer(1.0) as never);
    const s = useDeckStore.getState();
    s.loadTrack('A', 'a', { title: '', artist: '', duration: 180, thumbnailUrl: null });
    s.loadTrack('B', 'b', { title: '', artist: '', duration: 180, thumbnailUrl: null });
  ...
  it('clamps the phase seek target to >= 0 when phaseDelta would push it negative', () => {
    ...
    playerRegistry.register('A', playerA as never);
    playerRegistry.register('B', mockPlayer(0.05) as never);
    const s = useDeckStore.getState();
    s.loadTrack('A', 'a', { title: '', artist: '', duration: 180, thumbnailUrl: null });
    s.loadTrack('B', 'b', { title: '', artist: '', duration: 180, thumbnailUrl: null });
