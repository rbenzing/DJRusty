...
| When | Script | Command |
|---|---|---|
| **Phase start** — check upstream artifacts | `Get-ArtifactInventory.ps1` | `.claude/skills/orchestration-artifacts/scripts/Get-ArtifactInventory.ps1 -ProjectName "{project}" -Phase "architecture"` |
| **Before handoff** — validate quality gate | `Test-QualityGate.ps1` | `.claude/skills/quality-gate-checker/scripts/Test-QualityGate.ps1 -ProjectName "{project}" -Phase "planning"` |
| **At handoff** — generate handoff message | `New-Handoff.ps1` | `.claude/skills/orchestration-handoffs/scripts/New-Handoff.ps1 -From "Planner" -To "Developer" -ProjectName "{project}" -Findings "decision1","decision2"` |
...
Review the generated message, add **Total Stories**, **Total Tasks**, **Start With**, and **Critical Notes**, then deliver it.
